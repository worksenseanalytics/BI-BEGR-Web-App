...
export function OverviewView({ setActiveTab }: OverviewViewProps) {
  ...
  const cmlScopes = useMemo(() => {
    ...
    const kpPass = kpRecords.filter(r => r.cultureMaturityLevel >= targetMaturity).length;
    const kpwPass = kpwRecords.filter(r => r.cultureMaturityLevel >= targetMaturity).length;
  ...
  const kpiData = useMemo(() => {
    ...
    const kpDiff = cmlScopes.kp - targetMaturity;
    const kpwDiff = cmlScopes.kpw - targetMaturity;
    ...
    return [
      {
        ...
        subtext: `${cmlScopes.biPassingPercent.toFixed(1)}% unit memenuhi target (${cmlScopes.biPassingCount}/${cmlScopes.biCount})`,
        icon: Award,
        colorClass: "amber"
      },
      {
        ...
        comparisonLabel: `vs target acuan (${formatScore(targetMaturity)})`,
        subtext: `${cmlScopes.kpPassingPercent.toFixed(1)}% unit memenuhi target (${cmlScopes.kpPassingCount}/${cmlScopes.kpCount})`,
        icon: Building,
        colorClass: "indigo"
      },
      {
        name: "CML Perwakilan (KPw)",
        stat: formatScore(cmlScopes.kpw),
        compareLabel: "skala 5.00",
        change: `${kpwDiff >= 0 ? "+" : ""}${kpwDiff.toFixed(2)}`,
        changeType: kpwDiff >= 0 ? "positive" : "negative",
        comparisonLabel: `vs target acuan (${formatScore(targetMaturity)})`,
        subtext: `${cmlScopes.kpwPassingPercent.toFixed(1)}% unit memenuhi target (${cmlScopes.kpwPassingCount}/${cmlScopes.kpwCount})`,
        icon: Users,
        colorClass: "emerald"
      },
      {
        name: "Target Kelayakan Budker",
        stat: formatScore(targetMaturity),
        compareLabel: "standard CML",
        change: "ACUAN",
        changeType: "neutral",
        comparisonLabel: "batas minimum kelulusan",
        subtext: "Ambang batas kelayakan nasional",
        icon: Target,
        colorClass: "violet"
      }
    ];
  ...
  const cpChartData = useMemo(() => {
    ...
    const items = [
      { name: "SESPI OK x KPP TOP", shortName: "SESPI OK", "Skor Rata-rata": begrRecords.reduce((s, r) => s + r.skorAkhirSespiok, 0) / count, color: "#6366f1" },
      { name: "AKU KEREN x BTS YUK", shortName: "AKU KEREN", "Skor Rata-rata": begrRecords.reduce((s, r) => s + r.skorAkhirAkukeren, 0) / count, color: "#06b6d4" },
      { name: "ONE BIG FAMILY (OBF)", shortName: "OBF", "Skor Rata-rata": begrRecords.reduce((s, r) => s + r.skorAkhirObf, 0) / count, color: "#10b981" },
      { name: "3H x KEJORA", shortName: "3H", "Skor Rata-rata": begrRecords.reduce((s, r) => s + r.skorAkhir3h, 0) / count, color: "#fbbf24" },
      { name: "PINTER x PASKEUN", shortName: "PINTER", "Skor Rata-rata": begrRecords.reduce((s, r) => s + r.skorAkhirPinter, 0) / count, color: "#ec4899" }
    ];
    return items.map(d => ({
      ...d,
      "Skor Rata-rata": parseFloat(d["Skor Rata-rata"].toFixed(2))
    }));
  }, [begrRecords]);
  ...
  return (
    <div className="space-y-6">
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-[11.5px] font-bold mt-1.5 block leading-none">
                      ...
                      <span className="text-slate-400 dark:text-slate-500">lulus</span>
                    </span>
                  </button>
                );
              })}
          ...
          <div className="w-full flex-1 flex flex-col justify-between">
            ...
            <div className="h-64 w-full flex-1 min-h-[240px]">
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" className="dark:stroke-slate-800/40" />
                  <XAxis dataKey="parameter" tick={{ fill: "#64748b", fontSize: 10, fontWeight: "bold" }} axisLine={false} tickLine={false} />
                  <YAxis 
                    domain={isYAxisDynamic ? [dataMin => Math.max(0, dataMin - 0.2), dataMax => dataMax + 0.2] : [0, 5]} 
                  ...
                  {showTargetComparison ? (
                    <Bar dataKey="Target Acuan" fill="#94a3b8" opacity={0.35} radius={[4, 4, 0, 0]} />
                  ) : (
          ...
          <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800/80 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 shrink-0">
            <div className="flex items-center space-x-3">
              ...
              <label 
                ...
                onClick={() => setShowTargetComparison(!showTargetComparison)}
              >
                Bandingkan berdampingan dengan Target Acuan ({formatScore(targetMaturity)})
              </label>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-150 dark:border-slate-800/85 rounded-[28px] p-6 shadow-sm flex flex-col justify-between">
          <div className="mb-5">
            <h3 className="text-base font-bold font-sans text-slate-855 dark:text-slate-100 tracking-tight">Radar Dampak NNS</h3>
            <p className="text-[13px] text-slate-400 dark:text-slate-500 font-semibold">Penilaian dampak perilaku utama Core Values Bank Indonesia</p>
          </div>

          <div className="h-64 w-full flex items-center justify-center relative">
            <ResponsiveContainer width="100%" height="100%">
              <RadarChart cx="50%" cy="50%" outerRadius="70%" data={radarNNSData}>
                ...
                <PolarRadiusAxis angle={30} domain={[0, 5]} tick={{ fill: "#94a3b8", fontSize: 9.5, fontWeight: "600" }} axisLine={false} tickLine={false} />
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-150 dark:border-slate-800/85 rounded-[28px] p-6 shadow-sm flex flex-col justify-between">
          <div className="border-b border-slate-50 dark:border-slate-850 pb-2.5 mb-4">
            <h3 className="text-base font-bold font-sans text-slate-855 dark:text-slate-100 tracking-tight">Komparasi Kematangan KP vs KPw</h3>
            ...
            <li>
              ...
              <div className="flex items-center space-x-1.5 mt-1">
                ...
                <span className="rounded bg-emerald-500/10 dark:bg-emerald-500/25 px-1.5 py-0.5 text-xs font-bold text-emerald-600 dark:text-emerald-450">
                  {cmlScopes.kpPassingPercent.toFixed(0)}% lulus
                </span>
              </div>
            </li>
            <li>
              ...
              <div className="flex items-center space-x-1.5 mt-1">
                ...
                <span className="rounded bg-emerald-500/10 dark:bg-emerald-500/25 px-1.5 py-0.5 text-xs font-bold text-emerald-600 dark:text-emerald-450">
                  {cmlScopes.kpwPassingPercent.toFixed(0)}% lulus
                </span>
              </div>
            </li>
          </ul>
        ...
        <div className="md:col-span-3 bg-white dark:bg-slate-900 border border-slate-150 dark:border-slate-800/85 rounded-[28px] p-6 shadow-sm flex flex-col justify-between">
          <div>
            <div className="mb-4 border-b border-slate-50 dark:border-slate-850 pb-2.5">
              ...
              <p className="text-[13px] text-slate-400 dark:text-slate-500 mt-1 font-semibold">Rata-rata skor akhir nasional untuk 5 program utama kepatuhan budaya kerja</p>
            </div>
            <div className="h-64 w-full">
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <YAxis domain={[dataMin => Math.max(0, parseFloat((dataMin - 0.2).toFixed(2))), dataMax => Math.min(5, parseFloat((dataMax + 0.2).toFixed(2)))]} tick={{ fill: "#64748b", fontSize: 11.5 }} axisLine={false} tickLine={false} tickFormatter={(v) => typeof v === "number" ? parseFloat(v.toFixed(2)).toString() : v} />
                  <Tooltip content={<CustomTooltip />} />
                  <Bar dataKey="Skor Rata-rata" radius={[4, 4, 0, 0]}>
                    {cpChartData.map((entry, index) => (
                      <Cell key={`cell-cp-${index}`} fill={entry.color} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-slate-100 dark:border-slate-800 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-wider">
                    ...
                    <th className="py-2.5 text-right">Skor</th>
                  </tr>
                </thead>
                    ...
                    <tr key={index} className="border-b border-slate-50 dark:border-slate-800/30 last:border-b-0 hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-colors">
                      <td className="py-3 pr-2">
                        <div className="flex items-center space-x-2">
                          ...
                          <span className="font-extrabold text-slate-700 dark:text-slate-200">{entry.name}</span>
                        </div>
                      </td>
                      <td className="py-3 text-right">
                        <span className="font-extrabold font-sans text-[13px] text-slate-800 dark:text-slate-100 bg-slate-50 dark:bg-slate-800 px-2 py-0.5 rounded border border-slate-100 dark:border-slate-700/60">
                          {entry["Skor Rata-rata"].toFixed(2)}
                        </span>
                      </td>
                    </tr>
                  ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-150 dark:border-slate-800/85 rounded-[28px] p-6 shadow-sm flex flex-col justify-between">
          <div className="mb-5">
            <h3 className="text-base font-bold font-sans text-slate-855 dark:text-slate-100 tracking-tight">Top 3 Satker Kematangan</h3>
            <p className="text-[13px] text-slate-400 dark:text-slate-500 font-semibold">Satker teladan pencapaian kualifikasi budaya nasional tertinggi</p>
          </div>
                ...
                >
                  ...
                  <div className="relative z-10 flex items-center justify-between min-w-0">
                    <div className="min-w-0">
                      ...
                      {/* Satker Name */}
                      <p className={`font-extrabold text-slate-855 dark:text-slate-100 truncate leading-tight group-hover:text-amber-600 dark:group-hover:text-amber-400 transition-colors ${isFirst ? "text-[15px]" : "text-[14px]"}`}>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-150 dark:border-slate-800/85 rounded-[28px] p-6 shadow-sm flex flex-col justify-between">
          <div className="mb-5">
            <h3 className="text-base font-bold font-sans text-slate-855 dark:text-slate-100 tracking-tight">Butuh Akselerasi Budaya</h3>
            <p className="text-[13px] text-slate-400 dark:text-slate-500 font-semibold">Satker prioritas bimbingan pembinaan intensif</p>
          </div>
                ...
                >
                  ...
                  <div className="relative z-10 flex items-center justify-between min-w-0">
                    <div className="min-w-0">
                      ...
                      {/* Satker Name */}
                      <p className={`font-extrabold text-slate-855 dark:text-slate-100 truncate leading-tight group-hover:text-rose-600 dark:group-hover:text-rose-500 transition-colors ${isFirst ? "text-[15px]" : "text-[14px]"}`}>
                        {satker.satkerLengkap}
                      </p>
                      {/* Sub-info */}
                      <div className="flex items-center gap-2 mt-1.5 flex-wrap">
