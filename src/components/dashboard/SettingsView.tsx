...
export function SettingsView() {
  ...
  const handleSaveSettings = (e: React.FormEvent) => {
    ...
    try {
      let parsedTarget = parseFloat(targetMaturity.toString());
      if (isNaN(parsedTarget) || parsedTarget <= 0 || parsedTarget > 5) {
        parsedTarget = 4.00;
      }
      savePengaturanData({
        ...
        <div className="space-y-4 font-medium text-xs font-sans">
          ...
          <div className="space-y-1.5">
            ...
            <input
              type="text"
              required
              placeholder="Contoh: Divisi Pengembangan Budaya Kerja"
              value={adminUser}
          ...
          <div className="space-y-1.5">
            <label className="text-[11px] text-slate-400 font-bold uppercase tracking-wider">Target Kelayakan Budker (CML Target)</label>
            <input
              type="number"
              step="0.01"
              min="1.00"
            ...
            <p className="text-[10px] text-slate-400 dark:text-slate-500 font-semibold mt-1">
              Batas minimum skor Culture Maturity Level (CML) agar Satker dinyatakan "Memenuhi Target" (skala 1-5).
            </p>
          </div>
